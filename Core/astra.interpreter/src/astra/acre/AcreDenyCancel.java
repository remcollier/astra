...
public class AcreDenyCancel extends AbstractStatement {
	private Term cid;
	
	public AcreDenyCancel(String clazz, int[] data, Term cid) {
	...
	public StatementHandler getStatementHandler() {
		return new StatementHandler() {
			...
			public boolean execute(final Intention context) {
				...
				IACREMessage am = context.getAcreAPI().getConversationManager().failCancel( c_id.value() );
			      if ( am != null ) {
					...
					if (!MessageService.send(toSend)) {
			    	  ...
			    	  context.notifyDone("Failed to confirm cancellation of Conversation: " + c_id);
			      }
				return false;
			}
			...
			public Statement statement() {
				return AcreDenyCancel.this;
			}
			
			public String toString() {
				return "acre_deny_cancel(" + cid.toString() + ")";
			}
